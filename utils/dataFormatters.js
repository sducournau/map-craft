 ...
 * @param {File} file - GeoPackage file
 * @returns {Promise<Object>} - GeoJSON data
 */
export async function processGeoPackage(file) {
  try {
    const geoPackage = await GeoPackage.open(file);
    ...
    for (const tableName of featureTables) {
      ...
      while (featureIterator.hasNext()) {
        const feature = featureIterator.next();
    ...
    // If only one feature collection, return it directly
    if (featureCollections.length === 1) {
      return formatGeoJson(featureCollections[0]);
    }
    
    // Combine all feature collections
    const combinedFeatures = featureCollections.reduce((acc, fc) => {
    ...
    throw new Error(`Error processing GeoPackage: ${error.message}`);
  }
}
 ...
 * @returns {Promise<Object>} - GeoJSON data
 */
export async function processShapefile(arrayBuffer) {
  try {
    // Parse the ZIP file
    const zip = await JSZip.loadAsync(arrayBuffer);
    
    // Extract the shapefile content
    const shpFiles = {};
    
    // Gather all necessary files (.shp, .dbf, .prj, etc.)
    await Promise.all(Object.keys(zip.files).map(async (filename) => {
      const content = await zip.files[filename].async('arraybuffer');
      shpFiles[filename] = content;
    }));
    
    // Process the shapefile with shpjs
    const geojson = await shp.parseShp(shpFiles);
    
    return formatGeoJson(geojson);
  } catch (error) {
    throw new Error(`Error processing Shapefile: ${error.message}`);
  }
}